...
function showPendingTasks(response, convo) {
  ...
  convo.ask('Which of these tasks would you like to work on today? Just tell me which numbers, or say `' + NONE.word + '` :1234:', function (response, convo) {
  ...
  if (NONE.reg_exp.test(response.text)) {
    ...
    convo.ask(taskListMessage, [{
      ...
      callback: function callback(response, convo) {
        ...
        convo.ask('When you\'re ready to continue, just say `' + FINISH_WORD.word + '`', function (response, convo) {
...
function askForDayTasks(response, convo) {
  ...
  convo.ask('Then just tell me when you\'re done by saying `' + FINISH_WORD.word + '`', function (response, convo) {
