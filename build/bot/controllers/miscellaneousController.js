...
exports.default = function (controller) {
	...
	// TEST
	controller.on('test_message_send', function (bot, userID, message) {
	...
	controller.hears(['^(kevin)$', '^(kev[in]*)$'], 'direct_message', function (bot, message) {
	...
	controller.hears(controller.utterances.hears.number, 'direct_message', _hearMiddleware.numberLessThanTen, function (bot, message) {
		bot.reply(message, "That is a number less than 10!");
	});
	controller.hears(controller.utterances.hears.number, 'direct_message', _hearMiddleware.numberGreaterThanTen, function (bot, message) {
		bot.reply(message, "That is a number greater than 10!");
	});

	controller.hears(["^chip$"], 'direct_message', function (bot, message) {
		...
		}, (0, _botResponses.randomInt)(500, 1500));
	});
	...
	controller.hears(controller.utterances.hears.hello, 'direct_message', function (bot, message) {
		...
		controller.storage.users.get(message.user, function (err, user) {
			if (user && user.name) {
				...
				var response = (0, _botResponses.helloResponse)();
				bot.send({
					type: "typing",
					channel: message.channel
				});
  ...
  */
	...
	controller.hears(["let's start a (.*) conversation about (.*)"], ["direct_message"], function (bot, message) {
...
var _hearMiddleware = require('../middleware/hearMiddleware');
